...
**Status**: PRs 1 + 2 + 3 + 4 of 8 complete on disk (uncommitted; awaiting user diff review before push). PR 1 scaffolding green (Vite + TS + Vitest + Playwright + Zod + ESLint + Prettier + pnpm + GH Actions + CC0 LICENSE + ADRs 0001-0008 + 2 concept docs). PR 3 shell shipped with portal grid + path router + GH-Pages SPA fallback + 5 placeholder tiles + 1 shipped tile (5-in-a-row stub). PR 4 engine shipped pure (board + BFS + line-detect + length-graded-with-cascade scoring + seeded RNG + versioned save schema + balance config). Gates: typecheck OK, lint 0 warnings, 110/110 unit + contract tests pass, 6/6 Playwright e2e pass (shell smoke), build 17.5 KB gz total (under 50 KB Holy Law cap; first-playable-frame stays well under 150 KB v1 cap). Awaiting Decision A (motif silhouette pick) before PR 5 (UI). PR 5/6/7/8 not started.
