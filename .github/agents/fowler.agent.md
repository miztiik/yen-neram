...
- When asked where to document a change, use the living concept, how-to, reference, or subsystem doc that owns the current shape. If the team actively explored and rejected a real alternative with non-trivial reversal cost, add that rationale there. Do not create decision files to record gameplay tuning, UI polish, architecture notes, or current implementation shape.
