...
If a finding fits two destinations, pick the living doc that a future maintainer would search first. Do not create a separate decision-record file; put durable tradeoffs in the owning living doc. If it fits none, it probably is not durable - leave it in the plan-doc `CLOSED` sub-section and move on.
...
- New rationale only when the PR actively explored and rejected a real alternative that future work could accidentally reopen.
...
- **Distilling speculation.** Only distill what shipped. A considered approach we did not take goes in a living doc's rejected-experiment note only when it still protects future work.
